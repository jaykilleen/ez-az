...
class TreasureController < ApplicationController
  ...
  def new
    room = Room.create!(game_slug: "treasure-hunt", tv_token: SecureRandom.alphanumeric(8).upcase)
    redirect_to treasure_path(room.code)
  end

  def show
    @room = Room.active.find_by(code: params[:code].to_s.upcase)
    return redirect_to new_treasure_path unless @room
