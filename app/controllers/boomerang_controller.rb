...
class BoomerangController < ApplicationController
  ...
  def new
    room = Room.create!(game_slug: "boomerang-brawl", tv_token: SecureRandom.alphanumeric(8).upcase)
    redirect_to boomerang_path(room.code)
  end

  def show
    @room = Room.active.find_by(code: params[:code].to_s.upcase)
    return redirect_to new_boomerang_path unless @room
