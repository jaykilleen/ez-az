...
class SpotlightController < ApplicationController
  ...
  def new
    room = Room.create!(game_slug: "spotlight", tv_token: SecureRandom.alphanumeric(8).upcase)
    redirect_to spotlight_path(room.code)
  end

  def show
    @room = Room.active.find_by(code: params[:code].to_s.upcase)
    return redirect_to new_spotlight_path unless @room
